...
var crosshair_lerp_speed : float = 5
...
func _process(delta):
	...
	var point_collision : Vector3 = view_cast.get_collision_point()
	...
	crosshair.position = crosshair.position.lerp(new_crosshair_pos - crosshair.size / 2, crosshair_lerp_speed * delta)
